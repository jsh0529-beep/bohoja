...
import {CaseRole,ConsentStatus,DocumentStatus,JobStatus,PaymentStatus,Prisma,PublicationStatus,RequestStatus,SubscriptionStatus,UserStatus} from '@prisma/client';
...
async function overview(caseId:string,userId:string){
  ...
  const records:any[]=[
    ...careLogs.map(item=>({id:item.id,kind:'care-logs',data:{meal:item.meal,pain:item.reportedPain,note:item.note},createdAt:item.createdAt.toISOString(),authorName:authors[item.authorId]??'보호자'})),
...
async function handlePOST(request:NextRequest,context:RouteContext){
  ...
  if(kind&&['care-logs','handoffs','questions','expenses','discharge/items'].includes(kind)){
    ...
    if(kind==='care-logs'){const value=parsed(z.object({meal:z.string().optional(),pain:z.coerce.number().int().min(0).max(10).optional(),note:z.string().max(2000).optional()}),raw);if(isResponse(value))return value;item=await prisma.careLog.create({data:{caseId,authorId:user.id,meal:value.meal,reportedPain:value.pain,note:value.note}});}
...
export async function DELETE(request:NextRequest,context:RouteContext){await ensureBootstrap();const csrf=csrfFailure(request);if(csrf)return csrf;const limited=await rateLimit(request,'mutation',rawToken(request)??'anonymous',180,60_000);return limited??handleDELETE(request,context);}

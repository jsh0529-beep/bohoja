...
async function canManageCareLog(caseId:string,userId:string,authorId:string){
  ...
  const member=await membership(caseId,userId);return Boolean(member&&new Set<CaseRole>([CaseRole.OWNER,CaseRole.CO_ADMIN]).has(member.role)&&await permission(caseId,userId,'manage'));
}

const carePatchText=z.union([z.string().trim().max(2000),z.null()]).optional().transform(value=>typeof value==='string'?(value||null):value);
const careLogPatchSchema=z.object({
  restore:z.boolean().optional(),recordedAt:z.union([z.coerce.date(),z.null()]).optional(),meal:carePatchText,
  mealType:z.enum(['조식','중식','석식','간식','해당 없음','BREAKFAST','LUNCH','DINNER','SNACK','NONE']).nullable().optional().transform(value=>(<{[key:string]:string}>{BREAKFAST:'조식',LUNCH:'중식',DINNER:'석식',SNACK:'간식',NONE:'해당 없음'})[value??'']??value),
  mealAmount:carePatchText,hydration:carePatchText,temperature:z.union([z.coerce.number().min(30).max(45),z.null()]).optional(),pain:z.union([z.coerce.number().int().min(0).max(10),z.null()]).optional(),medication:carePatchText,bowelMovement:carePatchText,sleep:carePatchText,mobility:carePatchText,mood:carePatchText,neededItems:carePatchText,heardFromStaff:carePatchText,note:carePatchText,
});
const meaningfulCare=(value:Record<string,unknown>)=>Object.entries(value).some(([key,item])=>!['recordedAt','restore'].includes(key)&&item!==undefined&&item!==null&&item!=='');
...
async function overview(caseId:string,userId:string){
  ...
  const [members,invitations,documents,careLogs,handoffs,questions,expenses,plans,events,tasks,notifications,utilities,managementHistory]=await Promise.all([
    ...
    prisma.careLog.findMany({where:{caseId,deletedAt:null},orderBy:{recordedAt:'desc'}}),
    prisma.handoffReport.findMany({where:{caseId},include:{acknowledgements:true},orderBy:{createdAt:'desc'}}),
    ...
    prisma.caseUtilityEntry.findMany({where:{caseId,deletedAt:null},orderBy:[{occursAt:'asc'},{createdAt:'desc'}]}),
    new Set<CaseRole>([CaseRole.OWNER,CaseRole.CO_ADMIN]).has(member.role)?prisma.auditLog.findMany({where:{caseId,action:{in:['family_invited','invitation_revoked','invitation_accepted','case_member_role_changed','case_member_revoked','personal_data_exported']}},orderBy:{createdAt:'desc'},take:30}):Promise.resolve([]),
  ]);
  ...
  for(const item of tasks){if(item.assigneeId)userIds.add(item.assigneeId);if(item.completedById)userIds.add(item.completedById);}
  ...
  const records:any[]=[
    ...careLogs.map(item=>({id:item.id,kind:'care-logs',authorId:item.authorId,canManageRecord:item.authorId===userId||member.role===CaseRole.OWNER||member.role===CaseRole.CO_ADMIN,data:{recordedAt:item.recordedAt.toISOString(),meal:item.meal,pain:item.reportedPain,bowelMovement:item.bowelMovement,sleep:item.sleep,mobility:item.mobility,mood:item.mood,neededItems:item.neededItems,heardFromStaff:item.heardFromStaff,note:item.note,...((item.detailsJson as Record<string,unknown>|null)??{})},createdAt:item.createdAt.toISOString(),updatedAt:item.updatedAt.toISOString(),deletedAt:item.deletedAt?.toISOString(),authorName:authors[item.authorId]??'보호자'})),
    ...handoffs.map(item=>({id:item.id,kind:'handoffs',authorId:item.authorId,data:{...(item.contentJson as Record<string,unknown>),acknowledgedByMe:item.acknowledgements.some(ack=>ack.userId===userId),acknowledgementCount:item.acknowledgements.length,acknowledgedUserIds:item.acknowledgements.map(ack=>ack.userId)},createdAt:item.createdAt.toISOString(),authorName:authors[item.authorId]??'보호자'})),
  ...
  return {
    case:await caseDto(member),currentUserId:userId,
    members:members.map(item=>({caseId,userId:item.userId,isSelf:item.userId===userId,canChangeRole:[CaseRole.CO_ADMIN,CaseRole.CAREGIVER,CaseRole.VIEWER].some(role=>role!==item.role&&canAdministerMember(member.role,userId,item.role,item.userId,role)),canRemove:canAdministerMember(member.role,userId,item.role,item.userId),role:apiRole(item.role),name:item.user.profile?.displayName??'알 수 없는 구성원',email:canSeeMemberEmails||item.userId===userId?item.user.credential?.email??'':''})),
    invitations:invitations.map(item=>({id:item.id,email:item.inviteeEmail,role:apiRole(item.role),status:item.revokedAt?'REVOKED':item.acceptedAt?'ACCEPTED':'PENDING',expiresAt:item.expiresAt.toISOString()})),documents:await Promise.all(documents.map(documentDto)),
    events:events.map(item=>({id:item.id,title:item.title,startsAt:item.startsAt.toISOString(),timeUnspecified:item.timeUnspecified,endsAt:item.endsAt?.toISOString(),location:item.location,important:item.important,updatedAt:item.updatedAt.toISOString(),canManage:roleAllows(member.role,'write')})),
    tasks:tasks.map(item=>({id:item.id,title:item.title,description:item.description,status:item.status===TaskStatus.TODO?'OPEN':item.status,dueAt:item.dueAt?.toISOString(),assigneeId:item.assigneeId,assigneeName:item.assigneeId?authors[item.assigneeId]??null:null,completedAt:item.completedAt?.toISOString()??null,completedByName:item.completedById?authors[item.completedById]??null:null,updatedAt:item.updatedAt.toISOString(),priority:item.priority,canManage:roleAllows(member.role,'write')})),
    notifications:notifications.map(item=>({id:item.id,category:item.category,referenceId:item.referenceId,readAt:item.readAt?.toISOString()??null,createdAt:item.createdAt.toISOString()})),
    utilities:utilities.map(item=>({id:item.id,kind:item.kind,title:item.title,detail:item.detail,occursAt:item.occursAt?.toISOString()??null,createdById:item.createdById,createdAt:item.createdAt.toISOString(),canManage:item.createdById===userId||member.role===CaseRole.OWNER||member.role===CaseRole.CO_ADMIN})),
    managementHistory:managementHistory.map(item=>({id:item.id,action:item.action,at:item.createdAt.toISOString()})),records,
  };
}
...
async function handleGET(request:NextRequest,context:RouteContext){
  ...
  if(key==='preferences'){const items=await prisma.notificationPreference.findMany({where:{userId:user.id}});return ok({items:items.map(item=>({channel:item.channel,enabled:item.marketingEnabled||item.transactionalEnabled,quietHours:item.quietHoursJson}))});}
  if(key==='settings'){const items=await prisma.userSetting.findMany({where:{userId:user.id}});return ok({items:items.map(item=>({key:item.key,value:item.valueJson}))});}
  if(key==='cases'){
  ...
  if(key.startsWith('admin/')){
    ...
    if(key==='admin/analytics'){const items=await prisma.anonymousUsageCounter.findMany({orderBy:[{day:'desc'},{eventName:'asc'}],take:200});return ok({items});}
  }
...
async function handlePOST(request:NextRequest,context:RouteContext){
  ...
  if(key==='analytics'){
    const value=parsed(z.object({event:z.enum(['page_view','care_log_saved','handoff_sent','notification_opened','utility_saved','pdf_downloaded'])}),raw);if(isResponse(value))return value;
    const day=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Seoul'}).format(new Date());await prisma.anonymousUsageCounter.upsert({where:{eventName_day:{eventName:value.event,day}},update:{count:{increment:1}},create:{eventName:value.event,day,count:1}});return ok({accepted:true},202);
  }
  ...
  if(key==='invitations/accept'){
    ...
    const changed=await prisma.invitation.updateMany({where:{id:invitation.id,acceptedAt:null,revokedAt:null,expiresAt:{gt:new Date()}},data:{acceptedAt:new Date(),acceptedById:user.id}});if(changed.count!==1)return fail('이미 사용된 초대입니다.',410,'INVITATION_INVALID');await prisma.caseMember.upsert({where:{caseId_userId:{caseId:invitation.caseId,userId:user.id}},update:{role:invitation.role,revokedAt:null},create:{caseId:invitation.caseId,userId:user.id,role:invitation.role}});await audit('invitation_accepted',user.id,invitation.id,invitation.caseId);await notifyCase(invitation.caseId,user.id,'INVITATION_ACCEPTED',invitation.id);return ok({caseId:invitation.caseId,role:apiRole(invitation.role)});
  }
  const user=await auth(request);if(isResponse(user))return user;
  if(key==='notifications/read-all'){
    const changed=await prisma.notification.updateMany({where:{userId:user.id,readAt:null},data:{readAt:new Date()}});
    await audit('notifications_read_all',user.id,user.id);return ok({success:true,count:changed.count});
  }
  if(path[0]==='cases'&&path[2]==='handoffs'&&path[3]&&path[4]==='acknowledge'){
    if(!await permission(path[1],user.id,'read'))return fail('접근 권한이 없습니다.',403,'FORBIDDEN');
    const handoff=await prisma.handoffReport.findFirst({where:{id:path[3],caseId:path[1]}});if(!handoff)return fail('교대 브리핑을 찾을 수 없습니다.',404,'NOT_FOUND');
    await prisma.handoffAcknowledgement.upsert({where:{handoffReportId_userId:{handoffReportId:handoff.id,userId:user.id}},update:{acknowledgedAt:new Date()},create:{handoffReportId:handoff.id,userId:user.id}});
    const count=await prisma.handoffAcknowledgement.count({where:{handoffReportId:handoff.id}});await audit('handoff_acknowledged',user.id,handoff.id,handoff.caseId);return ok({success:true,count});
  }
  if(path[0]==='cases'&&path.length===3&&path[2]==='utilities'){
    if(!await permission(path[1],user.id,'write'))return fail('병원생활 정보를 작성할 권한이 없습니다.',403,'FORBIDDEN');
    const value=parsed(z.object({kind:z.enum(['HOSPITAL_INFO','SHIFT','SUPPLY_REQUEST']),title:z.string().trim().min(1).max(200),detail:z.string().trim().max(2000).optional(),occursAt:z.coerce.date().optional()}),raw);if(isResponse(value))return value;
    const item=await prisma.caseUtilityEntry.create({data:{caseId:path[1],kind:value.kind,title:value.title,detail:value.detail||null,occursAt:value.occursAt,createdById:user.id}});await audit('case_utility_created',user.id,item.id,item.caseId);await notifyCase(item.caseId,user.id,`UTILITY_${item.kind}`,item.id);return ok({item:{id:item.id,kind:item.kind,title:item.title,detail:item.detail,occursAt:item.occursAt?.toISOString()??null,createdAt:item.createdAt.toISOString()}},201);
  }
  ...
  if(path[0]==='cases'&&path.length===3&&path[2]==='events'){
    ...
    const item=await prisma.event.create({data:{caseId,title:value.title,startsAt:value.startsAt??new Date(),timeUnspecified:!value.startsAt,endsAt:value.endsAt,location:value.location||null,important:value.important,createdById:user.id}});await audit('event_created',user.id,item.id,caseId);await notifyCase(caseId,user.id,'EVENT_CREATED',item.id);return ok({event:{id:item.id,title:item.title,startsAt:item.startsAt.toISOString(),timeUnspecified:item.timeUnspecified,endsAt:item.endsAt?.toISOString(),location:item.location,important:item.important}},201);
  }
  ...
  if(kind&&['care-logs','handoffs','questions','expenses','discharge/items'].includes(kind)){
    ...
    if(kind==='handoffs'){
      const optional=z.string().trim().max(2000).optional().transform(value=>value||undefined);
      const value=parsed(z.object({status:optional,summary:optional,next:optional,question:optional,neededItems:optional}).refine(item=>Object.values(item).some(Boolean),{message:'전달할 내용을 한 가지 이상 입력해 주세요.'}),raw);if(isResponse(value))return value;
      item=await prisma.handoffReport.create({data:{caseId,authorId:user.id,contentJson:value as Prisma.InputJsonValue,periodStart:new Date(),periodEnd:new Date(Date.now()+86400000)}});
    }
...
async function handlePATCH(request:NextRequest,context:RouteContext){
  ...
  if(path[0]==='notifications'&&path[1]){
    const user=await auth(request);if(isResponse(user))return user;const value=parsed(z.object({read:z.boolean()}),raw);if(isResponse(value))return value;
    const item=await prisma.notification.findFirst({where:{id:path[1],userId:user.id}});if(!item)return fail('알림을 찾을 수 없습니다.',404,'NOT_FOUND');const updated=await prisma.notification.update({where:{id:item.id},data:{readAt:value.read?new Date():null}});await audit(value.read?'notification_read':'notification_unread',user.id,item.id);return ok({notification:{id:updated.id,readAt:updated.readAt?.toISOString()??null}});
  }
  if(path[0]==='cases'&&path[2]==='members'&&path[3]){
    const user=await auth(request);if(isResponse(user))return user;const [actor,target]=await Promise.all([membership(path[1],user.id),membership(path[1],path[3])]);if(!actor||!target)return fail('구성원을 찾을 수 없습니다.',404,'NOT_FOUND');const value=parsed(z.object({role:z.enum(['MANAGER','CO_ADMIN','CAREGIVER','VIEWER'])}),raw);if(isResponse(value))return value;const nextRole=dbRole(value.role);if(!canAdministerMember(actor.role,user.id,target.role,target.userId,nextRole))return fail('이 구성원의 권한을 변경할 수 없습니다.',403,'FORBIDDEN');const item=await prisma.caseMember.update({where:{id:target.id},data:{role:nextRole}});await prisma.notification.create({data:{userId:item.userId,category:'ROLE_CHANGED',channel:'IN_APP',templateKey:'role_changed',referenceId:item.caseId,dedupeKey:`role:${item.id}:${nextRole}:${Date.now()}`,status:'SENT',scheduledAt:new Date(),sentAt:new Date()}});await audit('case_member_role_changed',user.id,item.id,item.caseId);return ok({member:{caseId:item.caseId,userId:item.userId,role:apiRole(item.role)}});
  }
  if(path[0]==='cases'&&path[2]==='care-logs'&&path[3]){
    ...
    const merged:Record<string,unknown>={meal:value.meal===undefined?item.meal:value.meal,pain:value.pain===undefined?item.reportedPain:value.pain,bowelMovement:value.bowelMovement===undefined?item.bowelMovement:value.bowelMovement,sleep:value.sleep===undefined?item.sleep:value.sleep,mobility:value.mobility===undefined?item.mobility:value.mobility,mood:value.mood===undefined?item.mood:value.mood,neededItems:value.neededItems===undefined?item.neededItems:value.neededItems,heardFromStaff:value.heardFromStaff===undefined?item.heardFromStaff:value.heardFromStaff,note:value.note===undefined?item.note:value.note,...details};if(!meaningfulCare(merged))return fail('기록할 내용을 하나 이상 유지해 주세요.',400,'EMPTY_CARE_LOG');
    const updated=await prisma.careLog.update({where:{id:item.id},data:{recordedAt:value.recordedAt===undefined?undefined:value.recordedAt??item.recordedAt,meal:value.meal,reportedPain:value.pain,bowelMovement:value.bowelMovement,sleep:value.sleep,mobility:value.mobility,mood:value.mood,neededItems:value.neededItems,heardFromStaff:value.heardFromStaff,note:value.note,detailsJson:details as Prisma.InputJsonValue,deletedAt:value.restore?null:undefined}});await audit(value.restore?'care_log_restored':'care_log_updated',user.id,item.id,item.caseId);return ok({record:{id:updated.id,kind:'care-logs',authorId:updated.authorId,data:{recordedAt:updated.recordedAt.toISOString(),meal:updated.meal,pain:updated.reportedPain,bowelMovement:updated.bowelMovement,sleep:updated.sleep,mobility:updated.mobility,mood:updated.mood,neededItems:updated.neededItems,heardFromStaff:updated.heardFromStaff,note:updated.note,...((updated.detailsJson as Record<string,unknown>|null)??{})},createdAt:updated.createdAt.toISOString(),updatedAt:updated.updatedAt.toISOString(),deletedAt:updated.deletedAt?.toISOString()}});
  }
  if(path[0]==='cases'&&path[2]==='events'&&path[3]){
    const user=await auth(request);if(isResponse(user))return user;const event=await prisma.event.findFirst({where:{id:path[3],caseId:path[1],deletedAt:null}});if(!event)return fail('일정을 찾을 수 없습니다.',404,'NOT_FOUND');if(!await permission(event.caseId,user.id,'write'))return fail('일정 변경 권한이 없습니다.',403,'FORBIDDEN');
    const optionalText=z.union([z.string().trim(),z.null()]).optional();const value=parsed(z.object({title:z.string().trim().min(1).max(200).optional(),startsAt:z.union([z.coerce.date(),z.null()]).optional(),location:optionalText}).refine(item=>Object.values(item).some(field=>field!==undefined),{message:'변경할 내용을 입력해 주세요.'}),raw);if(isResponse(value))return value;const item=await prisma.event.update({where:{id:event.id},data:{title:value.title,startsAt:value.startsAt instanceof Date?value.startsAt:undefined,timeUnspecified:value.startsAt===undefined?undefined:value.startsAt===null,location:value.location===undefined?undefined:value.location||null}});await audit('event_updated',user.id,item.id,item.caseId);await notifyCase(item.caseId,user.id,'EVENT_UPDATED',item.id);return ok({event:{id:item.id,title:item.title,startsAt:item.startsAt.toISOString(),timeUnspecified:item.timeUnspecified,location:item.location,updatedAt:item.updatedAt.toISOString()}});
  }
  if((path[0]==='cases'&&path[2]==='tasks'&&path[3])||(path[0]==='tasks'&&path[1])){
    ...
    const value=parsed(z.object({title:z.string().trim().min(1).max(200).optional(),dueAt:z.union([z.coerce.date(),z.null()]).optional(),assigneeId:z.union([z.string().min(1),z.null()]).optional(),status:z.enum(['OPEN','TODO','DOING','DONE','CANCELLED']).optional()}).refine(item=>Object.values(item).some(field=>field!==undefined),{message:'변경할 내용을 입력해 주세요.'}),raw);if(isResponse(value))return value;
    if(value.assigneeId){const assignee=await membership(task.caseId,value.assigneeId);if(!assignee||!active(assignee))return fail('활성 돌봄방 구성원에게만 할 일을 배정할 수 있습니다.',400,'INVALID_ASSIGNEE');}
    const status=value.status?(value.status==='OPEN'?TaskStatus.TODO:value.status as TaskStatus):undefined;const done=status===TaskStatus.DONE;const statusChanged=status!==undefined&&status!==task.status;const item=await prisma.task.update({where:{id:task.id},data:{title:value.title,dueAt:value.dueAt===undefined?undefined:value.dueAt,assigneeId:value.assigneeId===undefined?undefined:value.assigneeId,status,completedAt:statusChanged?(done?new Date():null):undefined,completedById:statusChanged?(done?user.id:null):undefined}});const names=item.assigneeId||item.completedById?Object.fromEntries((await prisma.userProfile.findMany({where:{userId:{in:[item.assigneeId,item.completedById].filter(Boolean) as string[]}}})).map(profile=>[profile.userId,profile.displayName])):{};await audit(statusChanged?(status===TaskStatus.CANCELLED?'task_cancelled':done?'task_completed':'task_reopened'):'task_updated',user.id,item.id,item.caseId);await notifyCase(item.caseId,user.id,'TASK_UPDATED',item.id);return ok({task:{id:item.id,title:item.title,status:item.status===TaskStatus.TODO?'OPEN':item.status,dueAt:item.dueAt?.toISOString(),assigneeId:item.assigneeId,assigneeName:item.assigneeId?names[item.assigneeId]??null:null,completedAt:item.completedAt?.toISOString()??null,completedByName:item.completedById?names[item.completedById]??null:null,updatedAt:item.updatedAt.toISOString()}});
  }
  if(key==='preferences'){const user=await auth(request);if(isResponse(user))return user;const value=parsed(z.object({channel:z.string().min(1),enabled:z.boolean(),quietStart:z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).optional(),quietEnd:z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).optional()}),raw);if(isResponse(value))return value;const marketing=value.channel.startsWith('marketing');const quietHours=value.quietStart&&value.quietEnd?{start:value.quietStart,end:value.quietEnd,timezone:'Asia/Seoul'}:undefined;const item=await prisma.notificationPreference.upsert({where:{userId_channel:{userId:user.id,channel:value.channel}},update:{...(marketing?{marketingEnabled:value.enabled}:{transactionalEnabled:value.enabled}),...(quietHours?{quietHoursJson:quietHours}: {})},create:{userId:user.id,channel:value.channel,transactionalEnabled:marketing?true:value.enabled,marketingEnabled:marketing?value.enabled:false,quietHoursJson:quietHours}});await audit('preference_updated',user.id,value.channel);return ok({preference:{channel:item.channel,enabled:value.enabled,quietHours:item.quietHoursJson}});}
  if(key==='settings'){const user=await auth(request);if(isResponse(user))return user;const value=parsed(z.object({key:z.enum(['display']),value:z.object({largeText:z.boolean(),simpleMode:z.boolean()})}),raw);if(isResponse(value))return value;const item=await prisma.userSetting.upsert({where:{userId_key:{userId:user.id,key:value.key}},update:{valueJson:value.value},create:{userId:user.id,key:value.key,valueJson:value.value}});await audit('user_setting_updated',user.id,value.key);return ok({setting:{key:item.key,value:item.valueJson}});}
...
async function deleteScheduleResource(request:NextRequest,context:RouteContext){
  ...
  if(path[0]==='cases'&&path[2]==='utilities'&&path[3]){const user=await auth(request);if(isResponse(user))return user;const item=await prisma.caseUtilityEntry.findFirst({where:{id:path[3],caseId:path[1],deletedAt:null}});if(!item)return fail('병원생활 정보를 찾을 수 없습니다.',404,'NOT_FOUND');const member=await membership(item.caseId,user.id);if(!member||!await permission(item.caseId,user.id,'write')||item.createdById!==user.id&&!new Set<CaseRole>([CaseRole.OWNER,CaseRole.CO_ADMIN]).has(member.role))return fail('삭제 권한이 없습니다.',403,'FORBIDDEN');await prisma.caseUtilityEntry.update({where:{id:item.id},data:{deletedAt:new Date()}});await audit('case_utility_deleted',user.id,item.id,item.caseId);return ok({success:true});}

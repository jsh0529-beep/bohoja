...
async function overview(caseId:string,userId:string){
  ...
  const records:any[]=[
    ...
    ...handoffs.map(item=>({id:item.id,kind:'handoffs',data:{...(item.contentJson as Record<string,unknown>),acknowledgedByMe:item.acknowledgements.some(ack=>ack.userId===userId),acknowledgementCount:item.acknowledgements.length},createdAt:item.createdAt.toISOString(),authorName:authors[item.authorId]??'보호자'})),

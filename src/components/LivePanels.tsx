...
export function AdminPanel(){
  ...
  const [legal,setLegal]=useState<Legal[]>([]); const [notices,setNotices]=useState<Notice[]>([]); const [privacy,setPrivacy]=useState<Privacy[]>([]); const [audit,setAudit]=useState<Audit[]>([]); const [error,setError]=useState(''); const [message,setMessage]=useState('');
  const load=useCallback(async()=>{try{const [s,l,n,p,a]=await Promise.all([json<typeof stats>('/api/admin/dashboard'),json<{items:Legal[]}>('/api/admin/legal'),json<{items:Notice[]}>('/api/admin/notices'),json<{items:Privacy[]}>('/api/admin/privacy-requests'),json<{items:Audit[]}>('/api/admin/audit')]);setStats(s);setLegal(l.items);setNotices(n.items);setPrivacy(p.items);setAudit(a.items);setError('')}catch(e){setError(e instanceof Error?e.message:'관리자 데이터를 불러오지 못했습니다.')}},[]);
